function traj = LQRwdz3(tspan,cont,P,Atil,Btil,datastartmv,datalengthmv,autopart,targetdata,windownum,epsilon,DS,winstart,Q0)

%need to add Qf ,DS.btilde, DS.atilde
%Q = 1*cont.Q(1:size(targetdata,1),1:size(targetdata,1));%%%%%%%0607
n=size(Atil{1},1);
%Q=15*blkdiag(1*eye(n/2),0*eye(n/2));
%Q(2,2)=100;
...
%          0 0 0  0 0 0;];
% Q=Q6;
Q=Q0*epsilon^2*eye(n);
Rhat =1*cont.Rhat*(epsilon^2);
%Rhat =0.01*cont.Rhat;
Qf =10*Q;
ep   =epsilon;
%n    = size(DS.M,1);
...
for i=winstart:windownum
    ...
    bQ_N    = 2*ep*transpose(Qf*P{i})*xauto(:,end);%7
    bQ_k    = 2*ep*transpose(Q*P{i})*xauto;
